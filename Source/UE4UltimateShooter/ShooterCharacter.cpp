...
void AShooterCharacter::TurnAtRate(float Rate)
{
	...
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds()); // deg/sec * sec/fram
}
	...
	FVector& OutBeamLocation)
{
	...
	if (bCrosshairHit)
	{
		// Tentative bem location - still need to trace from gun
	...
	// const FVector WeaponTarceEnd{ OutBeamLocation };
	const FVector WeaponTarceEnd{ MuzzleSocketLocation + StartEnd };
	...
	if (bScreenToWorld) // was deprojection succesful?
	{
		...
		// Set Beam and point to line trace end point
		OutBeamLocation = End;
...
{
	CombatState = ECombatState::ECS_FireTimerInPorgress;
...
void AShooterCharacter::TraceForItems()
{
	...
	{
		// No longer overlapping anu item,
...
void AShooterCharacter::EquipWeapon(AWeapon* WeaponToEquip)
{
	if (WeaponToEquip)
	{
		...
		// FOR REMOVE WeaponToEquip->GetAreaSphere()->SetCollisionResponseToAllChannels(ECollisionResponse::ECR_Ignore);

		/** Set CollisionBox to ignore all Collision Channels */
		//TODO FOR REMOVE WeaponToEquip->GetCollisionBox()->SetCollisionResponseToAllChannels(ECollisionResponse::ECR_Ignore);

		// Get the hand Socket
		const USkeletalMeshSocket* HandSocket = GetMesh()->GetSocketByName(FName("RightRandSocket"));
		if (HandSocket)
		{
...
void AShooterCharacter::ReloadWeapon()
{
	...
	if (CarryingAmmo() && !EquippedWeapon->ClipsFull())
	{
		...
		UAnimInstance* AnimInstance = GetMesh()->GetAnimInstance();
		if (AnimInstance && ReloadMonatge)
		{
			AnimInstance->Montage_Play(ReloadMonatge);
...
void AShooterCharacter::GrabClip()
{
	...
	HandScceneComponent->AttachToComponent(GetMesh(), AttachmentRules, FName(TEXT("Hand_L")));
	HandScceneComponent->SetWorldTransform(ClipTransform);
...
void AShooterCharacter::FinishReloading()
{
	...
	if (AmmoMap.Contains(AmmoType))
	{
		// Ammount of ammo the Character is carrying of the EquippedWeapon types
		int32 CarriedAmmo = AmmoMap[AmmoType];

		// Space left in the megazine of EquippedWeapon
...
FVector AShooterCharacter::GetCameraInterpLocation()
{
	...
	return CameraWorldLocation + CameraForward * CameraInterpDistance + FVector(0.f, 0.f, CameraInterElevation);
}
